...
(function () {
  ...
  // Preferred local fallback chain for regular sans/serif text replacement.
  ...
  // Target local font for mono/code-like content.
  const MONO_LOCALS = [
    "Noto Sans Mono",
  ];

  // Target local font for mathematical characters.
  const MATH_LOCALS = [
    "Noto Sans Math",
  ];

  // Target local font chain for emoji characters.
  const EMOJI_LOCALS = [
    "Noto Color Emoji",
  ];
  ...
  function localsToSrc(locals) {
    return locals.map(name => `local('${escapeFontName(name)}')`).join(", ");
  }
  ...
  function buildFontFaceRule(family, src) {
    return [
      "@font-face {",
  ...
  function buildRegularFontFaceCSS(usages) {
    const src = localsToSrc(FALLBACK_LOCALS);

    return usages.map(({ family }) => (
      buildFontFaceRule(family, src)
    )).join("\n\n");
  }

  function buildSpecialFontFaceCSS(usageData) {
    const css = [];
    const monoSrc = localsToSrc(MONO_LOCALS);
    const mathSrc = localsToSrc(MATH_LOCALS);
    const emojiSrc = localsToSrc(EMOJI_LOCALS);

    if (ENABLE_MONO_REPLACEMENT) {
      for (const { family } of usageData.monoUsages) {
        css.push(buildFontFaceRule(family, monoSrc));
      }
    }

    if (ENABLE_MATH_REPLACEMENT) {
      for (const { family } of usageData.mathUsages) {
        css.push(buildFontFaceRule(family, mathSrc));
      }
    }

    if (ENABLE_EMOJI_REPLACEMENT) {
      for (const { family } of usageData.emojiUsages) {
        css.push(buildFontFaceRule(family, emojiSrc));
      }
    }
  ...
  function applyDynamicFontOverride() {
    ...
    const hasRegularRules = usageData.regularUsages.length > 0;
    const hasMonoRules = ENABLE_MONO_REPLACEMENT && usageData.monoUsages.length > 0;
    const hasMathRules = ENABLE_MATH_REPLACEMENT && usageData.mathUsages.length > 0;
    const hasEmojiRules = ENABLE_EMOJI_REPLACEMENT && usageData.emojiUsages.length > 0;
    ...
    const targetFamilies = collectReplacementFamilies(
      usageData.regularUsages,
      usageData.eligibleUsages,
      usageData.monoUsages,
      usageData.mathUsages,
      usageData.emojiUsages
    );
